...
// [FF-COMPAT] DEBUG provided by license.js
// [FF-COMPAT] log provided by license.js
...
try {
  ...
  console.warn('âš ï¸ [ONNX] Runtime failed to load:', e.message);
}
  ...
  _onnxInitPromise = (async () => {
    try {
      ...
      if (!modelResp) {
        ...
        log('âœ… [ONNX] Model downloaded and cached');
      }
      ...
      log('âœ… [ONNX] Session ready');
      return _onnxSession;
    } catch (err) {
      console.warn('âš ï¸ [ONNX] Init failed:', err.message);
...
async function fetchImagePixels(url) {
  try {
    ...
    log('âš¡ [ONNX] Pixel fetch failed:', e.message);
    return null;
  }
}
...
async function runLocalInference(url) {
  try {
    ...
    log(`âš¡ [ONNX] ai=${(aiScore * 100).toFixed(1)}%`);
    return { aiScore };
  } catch (err) {
    console.warn('âš ï¸ [ONNX] Inference error:', err.message);
    return null;
  }
}
...
function buildLocalResult(aiScore, isAI) {
  ...
  return {
    ...
    indicators: [
      isAI
        ? `AI confidence: ${pct}% â€” strong AI generation signals detected`
        : `Real confidence: ${realPct}% â€” no AI signals detected`,
      isAI
        ? 'â„¹ï¸ Scanned by local model â€” upgrade to Pro for full API breakdown'
        : 'â„¹ï¸ Note: Photo manipulation (face swaps, filters) can evade AI detectors â€” trust your instincts'
    ],
    localOnly: true,
    timestamp: Date.now()
  };
...
// [FF-COMPAT] BACKEND_URL provided by license.js
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  chrome.storage.local.get(['userId', 'license'], async (result) => {
    ...
    if (!result.license) {
      ...
      log('âœ… Free tier initialized - 3 scans/day');
    }
  });
});
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === 'local' && changes.license) {
    ...
    if (newLicense?.isPro) {
      log('âœ¨ Pro license activated:', newLicense.plan);
    } else if (changes.license.oldValue?.isPro) {
      log('ðŸ”“ Pro license deactivated');
    }
  }
});
...
async function analyzeVideo(request, callback) {
  try {
    ...
    if (error.name === 'AbortError') {
      ...
      console.error('âŒ analyzeVideo error:', error);
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === 'checkAI') {
    log('ðŸ–±ï¸ Context menu clicked for:', info.srcUrl);
...
async function processQueue() {
  ...
  if (timeSinceLastRequest < MIN_REQUEST_INTERVAL) {
    ...
    log(`â±ï¸ [RATE LIMIT] Waiting ${delay}ms...`);
...
async function processAnalysis(request) {
  ...
  log('ðŸ” Processing analysis for:', imageData.src?.substring(0, 50));
  ...
  const unavailableMsg = platformName
    ? `âš ï¸ Detection service temporarily unavailable on ${platformName}`
    : 'âš ï¸ Detection service temporarily unavailable';
  ...
  try {
    ...
    if (!src || src.startsWith('blob:') ||
      ...
      return {
        ...
        verdict: 'error',
        indicators: ["This image can't be scanned â€” it has no direct URL"]
      };
    }
    ...
    if (response.status === 429 || data.error === 'DAILY_LIMIT_REACHED') {
      ...
      return {
        ...
        indicators: [
          ...
          'ðŸ‘‰ Upgrade to Pro for unlimited'
        ],
        method: 'error',
    ...
    if (response.status === 402 || data.error === 'TOKENS_EXHAUSTED') {
      return {
        ...
        aiProbability: 0,
        confidence: 0,
        indicators: ['ðŸ”’ Token balance exhausted', 'Purchase more tokens to continue'],
        method: 'error',
        error: 'TOKENS_EXHAUSTED',
    ...
    if (!response.ok || !data.success) {
      console.warn('âš ï¸ Proxy returned error:', data);
      let userMessage;
      if (data.seCode === 1044 || data.error === 'UNSCANNABLE_URL') {
        userMessage = "This image doesn't have a direct URL â€” try opening the image in a new tab";
      } else if (data.seCode === 1201) {
        userMessage = "Image URL redirected and couldn't be resolved â€” try from the original source page";
        ...
        userMessage = 'Detection service is busy â€” try again in a moment';
      } else {
    ...
    console.error('âŒ Proxy call failed:', error);
    ...
    return {
      ...
      indicators: [
        platform
          ? `Could not reach Faux Spy on ${platform} â€” check your connection`
          : 'Backend unavailable â€” check your connection'
      ]
    };
  }
}
...
async function checkCache(imageUrl) {
  try {
    ...
    if (age > CACHE_DURATION) {
      log('ðŸ—‘ï¸ [CACHE] Expired, will re-analyze');
      return null;
    }
...
async function analyzeImageHeuristic({ src, width, height, pageUrl, pageHost, pageTitle }) {
  try {
    ...
    if (confidence >= 0.40 && confidence < 0.60) {
      indicators.push('âš ï¸ Inconclusive - configure Hive AI for accurate detection');
    } else if (confidence < 0.40) {
      indicators.push('Heuristic-only analysis - configure Hive AI for verification');
    }
    
    log(`ðŸ” [HEURISTIC] Final score: ${(confidence * 100).toFixed(1)}%`);
    log(`ðŸ” [HEURISTIC] Verdict: ${isAI ? 'AI' : (confidence >= 0.40 ? 'Inconclusive' : 'Real')}`);
    log(`ðŸ” [HEURISTIC] Indicators:`, indicators);
